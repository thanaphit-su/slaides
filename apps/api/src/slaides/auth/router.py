...
async def guest(body: GuestJoinRequest, session: AsyncSession = Depends(db_session)) -> GuestJoinResponse:
    ...
    row = (await session.execute(select(SessionRow).where(SessionRow.code == code))).scalar_one_or_none()
    if row is None:
    ...
    participant = (
        await session.execute(
            select(Participant).where(Participant.session_id == row.id, Participant.ref == ref)
        )
    ).scalar_one_or_none()
    if participant is None:
        ...
        existing = (
            await session.execute(
                select(func.count(Participant.id)).where(Participant.session_id == row.id)
            )
        ).scalar_one()
        if existing >= cap:
            raise HTTPException(
        ...
        await session.flush()
    token = service.issue_guest(participant.id, row.id, ref)
